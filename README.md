...
No utilizo threads porque en un Arduino UNO realmente no existe multitarea real, he tratado de imitar ese comportamiento estructurando mi programa como una máquina de estados no bloqueante.
...
Para gestionar temporizaciones precisas sin bloquear la ejecución, utilizo timers basados en micros(). Esto me permite medir intervalos de tiempo mientras el programa sigue atendiendo al joystick, actualizando el LCD o procesando entradas. Cada estado usa sus propias banderas (primeraVezTemp, primeraVezPreparando, etc.) junto a estos temporizadores para ejecutar acciones iniciales solo una vez y controlar correctamente los tiempos sin delay().
...
Respecto al watchdog: no utilizo watchdog, ya que mi diseño no lo requiere. Al no usar delays bloqueantes y tener una estructura totalmente controlada por estados, el flujo del programa nunca se queda colgado. Aun así, sería posible añadirlo como mecanismo de seguridad extra, pero en esta práctica no ha sido necesario.
